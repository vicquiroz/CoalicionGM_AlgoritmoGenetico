...
float eval_sol(int* pos, float** mat, int largo) {
	...
	for (size_t i = 0; i <= (largo - 2); i++)
	{
		...
		for (size_t j = i + 1; j <= (largo - 1); j++)		///Error salida de rango en llamada
		{
...
{
	int* temp = (int*)malloc(sizeof(int) * largo1);	//Error Alloc'd
	int cont = 0;
	bool flag = false;
...
//funcion principal
tuple<float,int> coalicionGM(int m, float pmutacion_threshold, float pr)
{
	//llamada de JSON
	...
	free(cromosoma);
	free(matDis);
	return make_tuple(bestSolution,it);
}